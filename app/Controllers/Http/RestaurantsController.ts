...
export default class RestaurantsController {
  ...
  public async listRestaurants({ response, params }: HttpContextContract) {
    ...
    const req = await fetch(
      `https://api.geoapify.com/v2/places?categories=catering.restaurant,catering.fast_food&filter=circle:${long},${lat},5000&bias=proximity:${long},${lat}&limit=100&apiKey=${key}`
    )
